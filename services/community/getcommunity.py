...
class community_detection():
    ...
    def getfeature_vectors(self):
        ...
        for segment in self.segments_list:
            for sent in segment['originalText']:
                graph_list[index] = (sent, formatTime(segment['startTime'], True), segment['spokenBy'], segment['id'])
    ...
    def edge_pruning(self, meeting_graph, graph_list, yetto_prune):
        yetto_prune = sorted(yetto_prune, key=lambda kv : kv[2], reverse=True)
        yetto_prune = yetto_prune[:math.ceil(len(yetto_prune)*0.01)+1]
    ...
    def louvian_community(self, meeting_graph_pruned):
    ...
    def refining_communities(self, community_set_sorted, graph_list):
        clusters = []
        temp = []
        prev_com = 0
    ...
    def groupbytime_communities(self, timerange):
       timerange_detailed = []
       temp = []
       flag = False
       pims = {}
    ...
    def wrapbytime_communities(self, pims):
        yet_to_combine = []
        need_to_remove = []
    ...
    def get_communities(self):
        ...
        fv, graph_list = self.getfeature_vectors()
        meeting_graph, yetto_prune = self.construct_graph(fv, graph_list)
        meeting_graph_pruned =  self.edge_pruning(meeting_graph, graph_list, yetto_prune)
        ...
        community_set_sorted = self.louvian_community(meeting_graph_pruned)
        community_timerange = self.refining_communities(community_set_sorted, graph_list)
       #logger.info("commnity timerange", extra={"timerange": community_timerange})
        pims = self.groupbytime_communities(community_timerange)
        pims = self.wrapbytime_communities(pims)
        return pims
