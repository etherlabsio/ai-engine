...
def get_score(mind_input, lambda_function):
    ...
    if response == 200:
        ...
        feature_vector, mind_vector, nsp_list = (
            np.array(data["sent_feats"][0]),
            np.array(data["mind_feats"][0]),
            data["sent_nsp_scores"][0],
        )

        if (
            lambda_function.lower() == "mind-01daaqyn9gbebc92aywnxedp0c"
            and len(feature_vector) > 0
        ):
        ...
        if len(feature_vector) > 0:
            for sent_vec, sent_nsp_list in zip(feature_vector, nsp_list):
                ...
                for mind_vec, mind_nsp in zip(mind_vector, sent_nsp_list):
                    sent_score_list.append(
                        get_cluster_score(sent_vec, mind_vec, mind_nsp)
                    )
                transcript_score_list.append(np.max(sent_score_list))
            transcript_score = np.mean(transcript_score_list)
    else:
