...
class KeyphraseExtractor(object):
    ...
    def get_entities(self, input_segment):
        ...
        for ent in list(set(doc.ents)):
            t_type = ent.label_
            if (
                t_type in list(match_dict)
                and str(ent) not in filtered_entities
            ):
    ...
    def segment_search(
        ...
        for i in range(len(input_json["segments"])):
            ...
            input_segment = (
                input_json["segments"][i].get("originalText").lower()
            )
            keywords_list = []
            ...
            segment_multiphrase_list = [
                words
                for words in segment_keyword_list
    ...
    def chapter_segment_offset_search(
        ...
        for i in range(len(segments)):
            ...
            entities = self.get_entities(entity_segment)
            chapter_entities.extend(
                [{entity: offset_time} for entity in entities]
            )

        chapter_entities_list = [
            entities
            for ent in chapter_entities
            for entities, off in ent.items()
        ]
