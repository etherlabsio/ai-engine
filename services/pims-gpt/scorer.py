...
def getScore(mind_input, lambda_function, mind_dict):
    ...
    if response == 200:
        ...
        if len(feature_vector) > 0:
            ...
            logger.warning('No valid sentences in the input text: {}'.format(mind_input))
            logger.warning('Returning default score')
    else:
        logger.warning('Invalid response from mind service for input: {}'.format(mind_input))
        logger.warning('Returning default score')
