...
def handler(event, context):
    ...
    try:
        ...
        if json_request['type'] == "segment_analyzer.extract_features":
            ...
            scores = list(map(lambda s: get_score(Request.mind_id, mind_dict, s, Request.context_id, Request.instance_id, for_pims=True), Request.segments))
            ...
            topics = {}
            pim = {}
            topics, pim = grouper.get_groups(Request_obj, lambda_function, mind_dict, for_pims=True)
            topics['contextId'] = Request_obj.context_id
            topics['instanceId'] = Request_obj.instance_id
            topics['mindId'] = Request_obj.mind_id
            output_pims = extra_preprocess.format_pims_output(pim, json_request['detail'], Request_obj.segments_map, Request_obj.mind_id)
